...
{
    public Pallet(List<Box> boxes, uint width, uint height, uint depth, uint weight = 30_000) : base(width, height, depth, weight)
    {
        Boxes = [];
        addBoxes(boxes);
        this.weight = weight;
    }
    public override uint Weight => (uint)Boxes.Sum(b => b.Weight) + weight;
    private uint weight;
